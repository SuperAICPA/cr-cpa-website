...
const translations = {
  en: {
    ...
    hero: {
      tagline: "Helping LA's Korean Businesses Keep More & Grow Faster",
  ...
  ko: {
    ...
    hero: {
      tagline: "한인 비즈니스의 절세와 성장을 함께 만드는 CPA",
...
export default function CRAccountancy() {
      ...
      <div style={baseStyles.navInner}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div style={{ display: "flex", background: `${colors.gold}15`, borderRadius: 6, border: `1px solid ${colors.gold}33`, overflow: "hidden" }}>
            {[{ code: "en", label: "EN" }, { code: "ko", label: "KO" }].map((l) => (
  ...
  const HomePage = () => (
    <div>
      ...
      <div style={{ ...baseStyles.section, paddingTop: 80, paddingBottom: 80 }}>
        ...
        {/* Persona Entry Cards — Individual vs Business */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 24, marginBottom: 56 }}>
          {[
            {
              icon: "👤",
              titleEn: "For Individuals", titleKo: "개인 고객",
              descEn: "Personal tax filing, foreign accounts, crypto, wealth & legacy planning",
              descKo: "개인 세금 신고 · 해외 계좌(FBAR) · 암호화폐 · 자산 및 상속 플래닝",
              links: [
                { id: "svc_1", en: "Tax Preparation", ko: "세금 신고" },
                { id: "svc_10", en: "International Tax (FBAR)", ko: "국제 세무 (FBAR)" },
                { id: "svc_11", en: "Crypto Tax", ko: "암호화폐 세금" },
                { id: "svc_7", en: "Wealth Planning", ko: "자산 플래닝" },
                { id: "svc_8", en: "Family Legacy", ko: "상속 계획" },
              ]
            },
            {
              icon: "🏢",
              titleEn: "For Business Owners", titleKo: "사업체 고객",
              descEn: "Bookkeeping, payroll, entity formation, consulting & industry expertise",
              descKo: "장부 관리 · 급여 · 법인 설립 · 컨설팅 · 업종별 전문 서비스",
              links: [
                { id: "svc_2", en: "Bookkeeping", ko: "장부 관리" },
                { id: "svc_3", en: "Payroll", ko: "급여 관리" },
            ...
            <div key={pi} style={{
              background: colors.white, border: `1px solid ${colors.lightGray}`, borderRadius: 8,
              padding: "32px 32px 24px", boxShadow: "0 4px 20px rgba(0,0,0,0.04)",
              borderTop: `4px solid ${pi === 0 ? colors.gold : colors.blue}`
            }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 12 }}>
                <span style={{ fontSize: 32 }}>{p.icon}</span>
                <h3 style={{ fontFamily: fonts.display, fontSize: 24, fontWeight: 700, color: colors.navy }}>
                  {lang === "ko" ? p.titleKo : p.titleEn}
                </h3>
              </div>
              <p style={{ fontSize: 13, color: colors.gray, lineHeight: 1.7, marginBottom: 18 }}>
                {lang === "ko" ? p.descKo : p.descEn}
              </p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {p.links.map((l, li) => (
                  <button key={li} onClick={() => navigate(l.id)} style={{
                    background: `${pi === 0 ? colors.gold : colors.blue}0d`,
                    border: `1px solid ${pi === 0 ? colors.gold : colors.blue}33`,
                    color: pi === 0 ? colors.gold : colors.blue,
                    fontFamily: fonts.sans, fontSize: 12, fontWeight: 600,
                    padding: "8px 14px", borderRadius: 100, cursor: "pointer", transition: "all 0.2s"
                  }}>
                    {lang === "ko" ? l.ko : l.en} →
                  </button>
                ))}
      ...
      {/* ── CR Consulting Division Spotlight ── */}
      <ConsultingSpotlight />
  ...
  // ============================================================
  // CR CONSULTING DIVISION SPOTLIGHT — links to consulting site
  // ============================================================
  const ConsultingSpotlight = () => (
    <div style={{ background: colors.darkNavy, position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", inset: 0, backgroundImage: `radial-gradient(circle at 80% 30%, ${colors.blue}22 0%, transparent 50%), radial-gradient(circle at 15% 80%, ${colors.gold}15 0%, transparent 45%)`, pointerEvents: "none" }} />
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "80px 40px", position: "relative" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 48, alignItems: "center" }}>
          {/* Left: Message */}
          <div>
            <div style={{ fontSize: 11, letterSpacing: 5, color: colors.gold, textTransform: "uppercase", marginBottom: 16, fontWeight: 700 }}>
              CR Consulting · {lang === "ko" ? "컨설팅 사업부" : "Consulting Division"}
            </div>
            <h2 style={{ fontFamily: fonts.display, fontSize: "clamp(28px, 3.5vw, 40px)", fontWeight: 700, color: colors.white, lineHeight: 1.25, marginBottom: 20 }}>
              {lang === "ko" ? "당신의 업종에 맞는 비즈니스 앱을 만들어주는 CPA" : "The CPA Who Builds Custom Apps for Your Industry"}
            </h2>
            <p style={{ fontFamily: fonts.body, fontSize: 16, color: `${colors.cream}bb`, lineHeight: 1.85, marginBottom: 28 }}>
              {lang === "ko"
                ? "건설업 Job Costing, 식당 재고·원가 관리, 급여 시스템까지 — 회계를 아는 CPA가 직접 설계하는 업종별 맞춤 앱 컨설팅. 스프레드시트를 벗어나 비즈니스 운영 시스템을 구축하세요."
                : "Construction job costing, restaurant inventory & food cost, payroll systems — custom business apps designed by a CPA who understands your numbers. Move beyond spreadsheets to a complete financial operating system."}
            </p>
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap", alignItems: "center" }}>
              <a href="https://consulting.dkcpala.com" target="_blank" rel="noopener noreferrer" style={{
                display: "inline-block", background: colors.gold, color: colors.darkNavy, textDecoration: "none",
                fontFamily: fonts.sans, fontSize: 13, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase",
                padding: "16px 36px", borderRadius: 4, boxShadow: `0 4px 24px ${colors.gold}44`
              }}>
                {lang === "ko" ? "컨설팅 사이트 방문 →" : "Visit Consulting Site →"}
              </a>
          ...
          <div style={{ display: "grid", gap: 14 }}>
            {[
              { icon: "🏗️", en: "Construction Job-Costing Apps", ko: "건설업 Job Costing 앱" },
              ...
              <div key={i} style={{
                ...
                background: `${colors.white}08`, border: `1px solid ${colors.white}12`,
